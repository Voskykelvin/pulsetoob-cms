...
export default function EditArticlePage() {
      ...
      api.get(`/articles/${id}`)
        .then(res => {
          if (res.data.success) {
            ...
            // Check and restore ID-specific local drafts so article data doesn't cross-pollinate
            const savedDraft = localStorage.getItem(`pulse_article_draft_${id}`)
            const savedImage = localStorage.getItem(`pulse_article_draft_image_${id}`)
            ...
            if (savedDraft) {
              ...
              if (confirmRestore) {
                try {
                  ...
                  localStorage.removeItem(`pulse_article_draft_${id}`)
                  localStorage.removeItem(`pulse_article_draft_image_${id}`)
                }
              } else {
                localStorage.removeItem(`pulse_article_draft_${id}`)
                localStorage.removeItem(`pulse_article_draft_image_${id}`)
  ...
  // Save Progress as Local Backup on Change (Scoped specifically to this article ID)
  useEffect(() => {
    if (id && !loading && (form.title || form.content || form.excerpt)) {
  ...
  useEffect(() => {
    if (id && !loading) {
      if (featuredImage) {
        localStorage.setItem(`pulse_article_draft_image_${id}`, JSON.stringify(featuredImage))
      } else {
        localStorage.removeItem(`pulse_article_draft_image_${id}`)
      }
    }
  }, [featuredImage, id, loading])
  ...
  const handleSubmit = async (status: 'draft' | 'published' | 'scheduled') => {
    ...
    try {
      ...
      if (res.data.success) {
        localStorage.removeItem(`pulse_article_draft_${id}`)
        localStorage.removeItem(`pulse_article_draft_image_${id}`)
        toast.success(status === 'published' ? 'Article updated and published' : status === 'scheduled' ? 'Article scheduled' : 'Article changes saved')
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-6">
              ...
              <div className="p-4 rounded-xl border border-gray-200 bg-gray-50">
                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Live Search Engine Snippet Preview</span>
